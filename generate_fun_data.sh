...
TMP_CACHE_DIR="${TMPDIR:-/tmp}/spectrometer_version_cache.$$"
...
# Create cache directory and output file path
rm -rf "${TMP_CACHE_DIR}"
...
# Step 6: Define tags to scan (in reverse-chronological order)
TAGS=(
    "v0.7.0-alpha.1"
    "v0.6.6"
    "v0.6.5"
...
if [ "$OUTPUT_FILE" -ef "$DEST_FILE" ]; then
    echo "Files are already the same, no copy needed"
else
    if [ -f "$DEST_FILE" ]; then
