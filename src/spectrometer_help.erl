...
usage_audit() ->
    io:format(
        ...
        "report which functions are NOT supported by AtomVM. The exit error value for this\n"
        "command is the number of unsupported functions. A negative exit value indicates an\n"
        "error occurred during the audit.\n"
        "\n"
        "Target (exactly one):\n"
        ...
        "  --dir <path>       Local directory containing .erl source files (including sub-dirs)\n"
...
usage_ecosystem() ->
    io:format(
        ...
        "\n"
        "State:\n"
        "  --resume           Resume from a previous audit, (--limit <N> will be added to the data)\n"
        "  --slow             With --resume, restart from page 1 to find items\n"
        "                     that shifted into previously-consumed pages\n"
