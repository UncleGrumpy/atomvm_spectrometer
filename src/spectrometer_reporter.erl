...
%% Returns binaries for matching against binary keys in stats maps.
-spec get_otp_module_set() -> sets:set(binary()).
get_otp_module_set() ->
    OtpMods = spectrometer_otp:modules_list(),
    %% Keep as binaries for matching against binary keys
    OtpBins = [list_to_binary(Mod) || Mod <- OtpMods],
    sets:from_list(OtpBins, [{version, 2}]).
        ...
        _ ->
            ...
            lists:foldl(
                fun({{Mod, Fun, Arity}, Count}, Idx) ->
                    MFAList =
                        binary_to_list(Mod) ++ ":" ++ binary_to_list(Fun) ++
                ...
                lists:foreach(
                    fun({{Mod, Fun, Arity}, Count}) ->
                        io:format(Fd, "~s,~s,~p,~p,no\n", [
                            quote_csv_field(binary_to_list(Mod)),
                            quote_csv_field(binary_to_list(Fun)),
                            Arity,
                            Count
                        ])
                    end,
