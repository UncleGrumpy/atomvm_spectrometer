...
parse_release_branch_version_basic_test_() ->
    {"parses major.minor version string into tuple", fun() ->
        ?assertEqual(
            {0, 7},
            spectrometer_updater:parse_release_branch_version(<<"0.7">>)
        ),
        ?assertEqual(
            {1, 2},
            spectrometer_updater:parse_release_branch_version(<<"1.2">>)
        ),
        ?assertEqual(
            {0, 6},
            spectrometer_updater:parse_release_branch_version(<<"0.6">>)
        )
    end}.

parse_release_branch_version_single_component_test_() ->
    {"parses single component version with minor=0", fun() ->
        ?assertEqual(
            {1, 0},
            spectrometer_updater:parse_release_branch_version(<<"1">>)
        ),
        ?assertEqual(
            {0, 0},
            spectrometer_updater:parse_release_branch_version(<<"0">>)
        )
    end}.

parse_release_branch_version_extra_components_test_() ->
    {"ignores extra version components beyond major.minor", fun() ->
        % e.g. "0.7.0" should still give {0, 7}
        ?assertEqual(
            {0, 7},
            spectrometer_updater:parse_release_branch_version(<<"0.7.0">>)
        ),
        ?assertEqual(
            {1, 2},
            spectrometer_updater:parse_release_branch_version(<<"1.2.3">>)
        )
    end}.

parse_release_branch_version_branch_sort_integration_test_() ->
    {"branch_sort_key uses parse_release_branch_version for release- branches", fun() ->
        % release-0.7 -> {2, {0, 7}} via parse_release_branch_version
        ?assertEqual(
            {2, {0, 7}},
            spectrometer_updater:branch_sort_key(<<"release-0.7">>)
        ),
        ...
        ?assertEqual(
            {2, {1, 0}},
            spectrometer_updater:branch_sort_key(<<"release-1.0">>)
        ),
        % Comparison: release-0.7 is older than release-1.0
        {2, {0, 7}} = spectrometer_updater:branch_sort_key(<<"release-0.7">>),
        {2, {1, 0}} = spectrometer_updater:branch_sort_key(<<"release-1.0">>),
        ?assert({2, {0, 7}} < {2, {1, 0}})
    end}.
